...
export async function generateReport(commonData: any) {
  ...
  if (!fs.existsSync(templatePath)) {
    console.warn("Template not found for australia:", templatePath);
    return null; 
  }
  ...
  const docxPath = path.join(generatedDir, `AUSTRALIA_Report_${timestamp}.docx`);
  fs.writeFileSync(docxPath, docxBuf);

  let pdfPath = null;
  try {
    const pdfBuf = await convertAsync(docxBuf, ".pdf", undefined);
    pdfPath = path.join(generatedDir, `AUSTRALIA_Report_${timestamp}.pdf`);
